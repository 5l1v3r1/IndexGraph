...
struct stinger * stinger_shared_new_full (char ** out, struct stinger_config_t * config)
{
//  printf("start to init all structs\n");
  if (*out == NULL)
  {
...
//  printf("nv-nebs-net\n");
  ...
  int64_t nv      = 1L << 2;
  int64_t nebs     = STINGER_DEFAULT_NEB_FACTOR * nv;
  int64_t netypes = STINGER_DEFAULT_NUMETYPES;
  int64_t nvtypes = STINGER_DEFAULT_NUMVTYPES;
...
//  printf("memory-size\n");
  const size_t memory_size = 1<<20;   // 1K
  ...
  if (!G) {
    printf("Failed to mmap STINGER graph.\n");
    exit(-1);
  }

  /* initialize the new data structure */
  xzero(G, sizeof(struct stinger) + sizes.size);
  //xzero(G, sizeof(struct stinger) + memory_size);
//  printf("xzero over\n");
