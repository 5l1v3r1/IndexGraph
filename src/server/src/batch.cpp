...
template<int64_t type>
inline void
handle_edge_names_types(EdgeInsertion & in, int64_t & src, int64_t & dest, int64_t & u, int64_t & v)
{
  if(type == MIXED) {
    if (in.has_source()) {
        printf("----------------here???\n");
    ...
    if (in.has_destination()) {
        ...
        dest = atol(in.destination_str().c_str());
        v = dest;
...
template <int64_t type>
void process_insertions(SharedBuffer * shared, VertexArray * va, StingerBatch & batch)
{
    size_t size_batch = batch.insertions_size();
    // printf("size batch is %ld\n", size_batch);
    #pragma omp parallel for schedule(static)
    for (size_t i = 0; i < size_batch; i++)
    {
        EdgeInsertion & in = *batch.mutable_insertions(i);
        int64_t u = -1, v = -1;
        u = atol(in.source_str().c_str());
        if(u != -1) in.set_source(u);

        v = atol(in.destination_str().c_str());
        if(v != -1) in.set_destination(v);
    ...
    if (batch.make_undirected())
    {
        stinger_batch_incr_edge_pairs<EdgeInsertionAdapter>(
                shared, va, batch.mutable_insertions()->begin(), batch.mutable_insertions()->end());
    } else {
 ...
 */
int
process_batch(SharedBuffer * shared, VertexArray * va, StingerBatch & batch)
{
    process_insertions<MIXED>(shared, va, batch);
